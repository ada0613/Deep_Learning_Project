...
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.conv1 = nn.Conv2d(in_channels=filters, out_channels=filters, stride=1, padding=1, kernel_size=3, bias=False)
        self.bn1 = nn.BatchNorm2d(num_features=filters, eps=1e-5, momentum=0.997)
        self.relu1 = nn.ReLU()
        self.conv2 = nn.Conv2d(in_channels=filters, out_channels=filters, stride=1, padding=1, kernel_size=3, bias=False)
...
class bottleneck_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.bn1 = nn.BatchNorm2d(num_features=out_channel, eps=1e-5, momentum=0.997)
        self.relu1 = nn.ReLU()
        ...
        self.relu2 = nn.ReLU()
        self.conv2 = nn.Conv2d(in_channels=out_channel, out_channels=out_channel, stride=1, padding=1, kernel_size=3, bias=False)
        self.bn3 = nn.BatchNorm2d(num_features=in_channel, eps=1e-5, momentum=0.997)
        self.relu3 = nn.ReLU()
        self.conv3 = nn.Conv2d(in_channels=out_channel, out_channels=in_channel, stride=1, kernel_size=1, bias=False)
        self.bn4 = nn.BatchNorm2d(num_features=in_channel, eps=1e-5, momentum=0.997)
        self.relu4= nn.ReLU()
        self.conv4 = nn.Conv2d(in_channels=in_channel, out_channels=in_channel, stride=1, kernel_size=1, bias=False)
        ...
        if block_fn is standard_block:
            if first_num_filters != filters:
                self.projection_shortcut = nn.Sequential(
                    nn.Conv2d(in_channels=filters_out // 2, out_channels=filters_out, stride=strides, kernel_size=1, bias=False),
            ...
            self.projection_shortcut = nn.Sequential(
                nn.Conv2d(in_channels=in_filters, out_channels=filters_out, stride=strides, kernel_size=1, bias=False),
                nn.BatchNorm2d(filters_out)
            )
